...
class ZeroX:
    def __init__(self, network: str, no_prompt=False, privkey_str: str = None, wallet_file: str = None,
        ...
        self._session.headers.update({'0x-api-key': self.api_key})
        self.acct = None
        if privkey_str and wallet_file:
            self.print.error('Specify either a privkey str or a json wallet file, not both!')
    ...
    def setup_w3(self, ):
        ...
        try:
            ...
            if conn:
                self.print.good(f"Connected to chain: {self.w3.eth.chain_id}")
            else:
                self.print.error(f'Web3 could not connect to remote endpoint: {w3_endpoint} , exiting!!')
                exit(1)
        if self.network == 'ethereum':
            self.endpoint = 'https://api.0x.org/'
            self.abi = libs.abi_lib.EIP20_ABI
        elif self.network == 'polygon':
            self.endpoint = 'https://polygon.api.0x.org/'
            self.abi = libs.abi_lib.EIP20_ABI
            self.w3.middleware_onion.inject(geth_poa_middleware, layer=0)
        elif self.network == 'bsc':
            self.abi = libs.abi_lib.BEP_ABI
            self.w3.middleware_onion.inject(geth_poa_middleware, layer=0)
            self.endpoint = 'https://bsc.api.0x.org/'
            self.print.warning('Connected to BSC, which has not been tested very well yet.')
        elif self.network == 'arbitrum':
            self.abi = libs.abi_lib.EIP20_ABI
            self.endpoint = 'https://arbitrum.api.0x.org/'

        self.print.good(f'Web3 connected to chain: {self.w3.eth.chain_id}')
        return self.w3
    ...
    def broadcast_tx(self, raw_txn: dict):
        ...
        signed_txn = self.w3.eth.account.sign_transaction(raw_txn, self.acct.key)
        try:
            ret = self.w3.eth.send_raw_transaction(signed_txn.rawTransaction)
        except ValueError as err:
            self.print.error(f'Error sending: {err}')
            hextx = False
        else:
            hextx = to_hex(ret)
            self.print.good(f'Transaction sent okay! Txid is: {hextx}')
        return hextx
    ...
    def poll_receipt(self, tx_hash):
        ...
        while True:
            if poll > 100:
                break
            poll += 1
            self.print.normal(f'Polling for receipt: {poll}/100 ... ')
            try:
                ...
                elapsed = end - start
                self.print.good(f'Confirmed in {elapsed} secs!')
                return receipt
        print('Timed Out!')
    ...
    def approve(self, token, amount=0):
        ...
        if not amount:
            ...
            max_amount = amount
        contract = self.w3.eth.contract(to_checksum_address(token), abi=libs.abi_lib.BEP_ABI)
        if self.network == 'bsc':
            tx = {
    ...
    def quote(self, buy_token: str, sell_token: str, raw_amount: int, quote_only=False):
        if args.disable_slippage_protection:
            slippage_protect = 'false'
        else:
            slippage_protect = 'true'
        params = {'sellToken': sell_token, 'buyToken': buy_token, 'sellAmount': raw_amount,
                  'takerAddress': self.acct.address, 'enableSlippageProtection': slippage_protect}
        self.print.normal(f'Get quote for IN: {sell_token}, OUT: {buy_token}')
        if quote_only:
        ...
        if resp.status_code == 200:
            ...
            if resp.status_code == 400:
                print(resp.text)
                if not quote_only and re.findall('allowance', resp.text):
                    self.print.error('Token probably needs approval')
                    ret = self.approve(sell_token)
                    self.poll_receipt(ret)
                    self.quote(buy_token, sell_token, raw_amount)

            self.print.warning(f'Non 200 response from 0x api: {resp.text}')
            return False
    ...
    def swap(self, buy_token, sell_token, raw_amount):
        ...
        try:
            ...
            self.print.error(f'Logic error: {err}')
            return 0
        ...
        if obj:
            ...
            if not self.no_prompt:
                ...
                if confirm.upper() == 'Y' or confirm.upper() == 'YES':
                    self.print.normal('Broadcasting transaction ... ')
                    self.broadcast_tx(tx)
                else:
                    self.print.normal('Operation canceled by user.')
            else:
                self.print.normal('Broadcasting transaction ... ')
                self.broadcast_tx(tx)
...
if __name__ == '__main__':
    ...
    args.add_argument('-n', '--network', dest='network_name', type=str, choices=['ethereum', 'polygon', 'bsc'],
    ...
    args.add_argument('-ds', '--disable-slippage-protection', dest='disable_slippage_protection', action='store_true')
    ...
    dotenv.load_dotenv()
    zrx_api_key = os.environ.get('zrx_key')
    if zrx_api_key is None:
        print('[!] Please set `zrx_key` in .env, as ZRX now requires an API key to use, which is rather '
              'unfortunate IMO.')
    ...
    api = ZeroX(args.network_name, args.no_prompt, args.privkey_as_str, args.json_wallet_file, zrx_api_key)
    api.print.good(f'API Configured, Network: {args.network_name}, Force: {args.no_prompt}, '
                    f'Wallet: {args.json_wallet_file}')
    if args.native_balance:
        raw, human = api.balance_check(contract_address=None)
        api.print.good(f'Balance: {human}, Raw: {raw}')
    ...
    if unlimited_approvals:
        api.print.warning('Warning: Unlimited Token Approvals Enabled.')
    if args.input_token and args.output_token:
        if not args.quantity:
            args.quantity, human_balance = api.balance_check(args.input_token)
            api.print.warning(f'Using full balance: {human_balance} , Raw: {args.quantity} as sell amount')
        if not args.quote_only:
            api.swap(args.output_token, args.input_token, args.quantity)
        else:
